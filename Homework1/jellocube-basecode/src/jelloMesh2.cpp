...
double JelloMesh2::g_structuralKs = 4500.0;
double JelloMesh2::g_structuralKd = 4.0;
double JelloMesh2::g_shearKs = 4500.0;
double JelloMesh2::g_shearKd = 3.0;
double JelloMesh2::g_bendKs = 5000.0;
double JelloMesh2::g_bendKd = 5.0;
double JelloMesh2::g_penaltyKs = 4500.0;
double JelloMesh2::g_penaltyKd = 6.0;

JelloMesh2::JelloMesh2() :
	m_integrationType(JelloMesh2::RK4), m_drawflags(MESH | STRUCTURAL),
...
void JelloMesh2::Reset()
{
	InitJelloMesh2();
}

JelloMesh2::Particle& JelloMesh2::GetParticle(JelloMesh2::ParticleGrid& grid, int i, int j, int k)
{
	return grid[i][j][k];
}

JelloMesh2::Particle& JelloMesh2::GetParticle(JelloMesh2::ParticleGrid& grid, int idx)
{
	int i, j, k;
	GetCell(idx, i, j, k);
...
const JelloMesh2::Particle& JelloMesh2::GetParticle(const JelloMesh2::ParticleGrid& grid, int i, int j, int k) const
{
	return grid[i][j][k];
}

const JelloMesh2::Particle& JelloMesh2::GetParticle(const JelloMesh2::ParticleGrid& grid, int idx) const
{
	int i, j, k;
	GetCell(idx, i, j, k);
	return GetParticle(grid, i, j, k);
...
bool JelloMesh2::isInterior(const JelloMesh2::Spring& s) const
{
...
bool JelloMesh2::isInterior(int idx) const
{
...
bool JelloMesh2::isInterior(int i, int j, int k) const
{
	return (i*j*k*(m_rows - i)*(m_cols - j)*(m_stacks - k) != 0);
}

void JelloMesh2::SetGridSize(int cols, int rows, int stacks)
{
	m_cols = cols;
	m_rows = rows;
	m_stacks = stacks;
...
int JelloMesh2::GetGridCols() const
{
	return m_cols;
}

int JelloMesh2::GetGridRows() const
{
	return m_rows;
}

int JelloMesh2::GetGridStacks() const
{
	return m_stacks;
}

void JelloMesh2::SetSize(float width, float height, float depth)
{
	m_width = width;
	m_height = height;
	m_depth = depth;
...
float JelloMesh2::GetWidth() const
{
	return m_width;
}
float JelloMesh2::GetHeight() const
{
	return m_height;
}

float JelloMesh2::GetDepth() const
{
	return m_depth;
}

int JelloMesh2::GetIndex(int i, int j, int k) const
{
	int cols = j;
	int rows = i*(m_rows + 1);
...
void JelloMesh2::GetCell(int idx, int& i, int &j, int& k) const
{
	float rows = m_rows + 1;
	float cols = m_cols + 1;
...
void JelloMesh2::InitJelloMesh2()
{
	m_vsprings.clear();
...
void JelloMesh2::AddStructuralSpring(Particle& p1, Particle& p2)
{
...
void JelloMesh2::AddBendSpring(JelloMesh2::Particle& p1, JelloMesh2::Particle& p2)
{
	double restLen = (p1.position - p2.position).Length();
	m_vsprings.push_back(Spring(BEND, p1.index, p2.index, g_bendKs, g_bendKd, restLen));
}

void JelloMesh2::AddShearSpring(JelloMesh2::Particle& p1, JelloMesh2::Particle& p2)
{
	double restLen = (p1.position - p2.position).Length();
	m_vsprings.push_back(Spring(SHEAR, p1.index, p2.index, g_shearKs, g_shearKd, restLen));
}

void JelloMesh2::SetIntegrationType(JelloMesh2::IntegrationType type)
{
	m_integrationType = type;
}

JelloMesh2::IntegrationType JelloMesh2::GetIntegrationType() const
{
	return m_integrationType;
}

void JelloMesh2::SetDrawFlags(unsigned int flags)
{
	m_drawflags = flags;
}

unsigned int JelloMesh2::GetDrawFlags() const
{
	return m_drawflags;
}

void JelloMesh2::DrawMesh(const vec3& eyePos)
{
	const ParticleGrid& g = m_vparticles;
	float red[4] = { 1.0,0.4,0.4,0.8 };
	float white[4] = { 1.0,1.0,1.0,1.0 };
	float purple[4] = { 0.9,0.2,1.0,1.0 };
	float blue[4] = { 0.0, 0.3,2.0, 5.0 };
...
void JelloMesh2::DrawSprings(double a)
{
	const ParticleGrid& g = m_vparticles;
	glBegin(GL_LINES);
...
void JelloMesh2::DrawCollisionNormals()
{
	const ParticleGrid& g = m_vparticles;
	glBegin(GL_LINES);
...
void JelloMesh2::Draw(const vec3& eyePos)
{
...
void JelloMesh2::Update(double dt, const World& world, const vec3& externalForces)
{
...
void JelloMesh2::CheckForCollisions(ParticleGrid& grid, const World& world)
{
	m_vcontacts.clear();
	m_vcollisions.clear();
...
void JelloMesh2::ComputeForces(ParticleGrid& grid)
{
...
void JelloMesh2::ResolveContacts(ParticleGrid& grid)
{
...
void JelloMesh2::ResolveCollisions(ParticleGrid& grid)
{
	for (unsigned int i = 0; i < m_vcollisions.size(); i++)
	{
...
bool JelloMesh2::FloorIntersection(Particle& p, Intersection& intersection)
{
	...
	if (p.position[1] <= 0.0)
	{
		...
		intersection.m_type = JelloMesh2::CONTACT;
		return true;
	}
	...
	else if (p.position[1] >= 0.0 && p.position[1] <= 0.0 + COLLISION_DELTA)
	{
		...
		intersection.m_normal = vec3(0.0, 1.0, 0.0); //normal vector-perpendicular to resting plane
		intersection.m_type = JelloMesh2::COLLISION;
		return true;
	}
	else { return false; }
}

bool JelloMesh2::CylinderIntersection(Particle& p, World::Cylinder* cylinder,
	JelloMesh2::Intersection& result)
{
...
void JelloMesh2::EulerIntegrate(double dt) // Y1=Y0+h f(X0,Y0); Yn+1 = Y+hf(Xn,Yn)
{ 
...
void JelloMesh2::MidPointIntegrate(double dt)
{
...
void JelloMesh2::RK4Integrate(double dt)
{
...
JelloMesh2::Spring::Spring() :
	m_type(JelloMesh2::STRUCTURAL),
	m_p1(-1),
	m_p2(-1),
...
JelloMesh2::Spring::Spring(const JelloMesh2::Spring& p) :
	m_type(p.m_type), m_p1(p.m_p1), m_p2(p.m_p2),
	m_Ks(p.m_Ks), m_Kd(p.m_Kd), m_restLen(p.m_restLen)
{
}

JelloMesh2::Spring& JelloMesh2::Spring::operator=(const JelloMesh2::Spring& p)
{
...
JelloMesh2::Spring::Spring(JelloMesh2::SpringType t,
...
JelloMesh2::Particle JelloMesh2::Particle::EMPTY;

JelloMesh2::Particle::Particle(int idx, const vec3& p, const vec3& v, double m)
{
	index = idx;
	position = p;
	velocity = v;
...
JelloMesh2::Particle::Particle() : index(-1), position(0, 0, 0), velocity(0, 0, 0), force(0, 0, 0), mass(1.0)
{
}

JelloMesh2::Particle::Particle(const JelloMesh2::Particle& p) :
	index(p.index), position(p.position), velocity(p.velocity), force(p.force), mass(p.mass)
{
}

JelloMesh2::Particle& JelloMesh2::Particle::operator=(const JelloMesh2::Particle& p)
{
	if (&p == this) return *this;
...
JelloMesh2::Intersection::Intersection() :
...
JelloMesh2::Intersection::Intersection(const JelloMesh2::Intersection& p) :
...
JelloMesh2::Intersection& JelloMesh2::Intersection::operator=(const JelloMesh2::Intersection& p)
...
JelloMesh2::Intersection::Intersection(IntersectionType type, int p, const vec3& normal, double d) :
...
void JelloMesh2::FaceMesh::Draw(const JelloMesh2& m)
...
void JelloMesh2::FaceMesh::DrawNormals(const JelloMesh2& m)
...
JelloMesh2::FaceMesh::FaceMesh(const JelloMesh2& m, JelloMesh2::Face f)
...
void JelloMesh2::FaceMesh::CalcDistToEye(const JelloMesh2& m, const vec3& eyePos)
...
bool JelloMesh2::FaceMesh::compare(const FaceMesh& one, const FaceMesh& other)
