...
double JelloMesh::g_bendKd = 4.75;
	...
	for (int i = 0; i < m_rows + 1; i++)
	{
		for (int j = 0; j < m_cols + 1; j++)
		{
			for (int k = 0; k < m_stacks + 1; k++)
			{
				...
				m_vparticles[i][j][k] = Particle(GetIndex(i, j, k), vec3(x, y+1, z));
			}
		}
	}
...
void JelloMesh::EulerIntegrate(double dt) // Y1=Y0+h f(X0,Y0); Yn+1 = Y+hf(Xn,Yn)
{ 
